...
> 仓库：`kechuang-mentor`
> 类型：AI 引导式科创项目陪练产品（**AI 引导老师 = 大老师方法论助教**）
...
**给学生用的"苏格拉底型 AI 科创项目导师"**——不直接给答案，通过项目流程和右下角大老师助教，引导孩子从生活观察出发，完成开题交流、背景调研、项目设计、方案设计、测试优化和申报材料整理。
...
| 1 | **开题交流** | `OPENING` | 通过对答了解学生背景、兴趣、经历和真实观察 | 可继续追问的现象 / 问题起点 |
| 2 | **背景调研** | `RESEARCH` | 用公开案例和资料启发学生理解好项目怎么提出问题 | 案例启发 + 资料线索 |
| 3 | **项目设计** | `DESIGN` | 反问、对比、假设，引导聚焦到可研究的具体问题 | 3-5 个候选方向 + 收敛到 1 个 |
| 4 | **方案设计** | `PLAN` | 引导调研计划、实验/开发设计、时间线、资源清单 | 完整研究方案骨架 |
| 5 | **测试优化** | `TEST` | 后端调用评估引擎或测试清单，**孩子端只给鼓励性反馈** | 改进建议 + 下一步测试动作 |
| 6 | **申报材料** | `SUBMISSION` | 整理课题设计，可导出"我的科创想法卡" | 4 格式导出（MD / TXT / HTML / PDF） |
...
内部可以根据学生状态调整提问侧重（问感受、问证据、问取舍、问进度），但不向学生暴露多人格切换，也不使用“暖心学姐”等角色名。
...
主人已从 PromptX 云端导出"大老师"全量人设。学生端只使用其中的方法论、判断标准和表达风格：
...
- 注入位置：
  ...
  2. **`src/data/personalities.js`** —— 学生端只保留一个"大老师"角色,按步骤和学生状态自动调整提问侧重
  3. **`src/data/cases.js`** —— 学生端只展示公开案例,不展示内部教学案例或个人经历
...
完整走一遍：破冰 → 开题交流 → 背景调研 → 项目设计 → 方案设计 → 测试优化 → 申报材料导出。
